...
void print_chessboard_and_pieces(size_t s, int n, bool p) {
	if (s == 64) {
		char l = 'a';
		printf("\n  ");
		while (l != 'i') printf("%c ", l++);
		return;
	}
	else if (!s) 		printf("%d ", n--);
	else if (!(s % 8)) 	printf("\n%d ", n--);
	else 			p = !p;
 	printf("%s%s%s "DEFAULT, 
			p ? BLACK_BACKGROUND : WHITE_BACKGROUND, 
...
int get_rules(char* f, char* t) {
	...
	if (from_position > 16 && from_position < 25 || from_position > 8 && from_position < 17)
		...
		return bishop(f, t);
	else if (from_position == 26 || from_position == 31 || from_position == 2 || from_position == 7)
		return knight();
	else if (from_position == 25 || from_position == 32 || from_position == 1 || from_position == 8)
		return rook(f, t);
...
int get_position_from_value(int p) {
	for (int i = 0; i < 64; ++i) if (board[i] == p) 	return i;
	return 0;	
}

int get_position_from_char(char* p) {
	for (int i = 0; i < 64; ++i) if (!strcmp(AN[i], p)) 	return i;
	return 0;
}
...
int pawn(void) {
	...
	if (player) {
		...
		if (to_position && to_position < 17 && to_char == from_char || to_value < from_value)
			return 0;
	}
	else {
	...
	if (two_square) {
		if (player) {
			...
			if (to_value != 6 && to_value != 5) 		return 0;
		}
	}
	else {
		if (player) {
	...
	if (to_char != from_char) {
		if (to_char != (from_char + 1) && to_char != (from_char - 1) || from_value == to_value)
			return 0;
		if (player) {	
			if (to_position > 16 || to_value > (from_value + 1)) 			return 0;
		}
		else {
			if (to_position && to_position < 17 || to_value < (from_value - 1)) 	return 0;
		}
		if      (!to_position) return 0;
	}
	return 1;
}
...
int bishop(char* f, char* t) {
	...
	if (to_char == from_char || v == 1 || v && v != get_position_from_char(t))	return 0;
	int  c = 0; 
	char l;
	if (to_char > from_char) {
		l = from_char;
...
int rook(char* f, char* t) {
	...
	if (to_value != from_value && to_char != from_char || v == 1 || v && v != get_position_from_char(t))
		return 0;
	return 1;
}

int queen(char* f, char* t) {
	if (!rook(f, t) && !bishop(f, t)) return 0;
	return 1;
}
...
void print_capture_pieces(void) {
	if (player) { for (int i = 0; i < white_captures_counter; ++i) 	printf("%s ", PIECES[white_captures[i]]); }
	else 	    { for (int i = 0; i < black_captures_counter; ++i) 	printf("%s ", PIECES[black_captures[i]]); }
	if (player && white_captures_counter || !player && black_captures_counter) puts("");
}

void set_bitboard(char* f) {
    	for (size_t i = 0; i < 64; ++i) {
       		if (get_rules(f, AN[i]) == 1) 	bitboard[i] = 1;
       		else 				bitboard[i] = 0;
    	}
}

void print_bitboard(void) {
    	for (size_t i = 0; i < 64; ++i) { if (i  % 8 == 0) puts(""); printf("%d ", bitboard[i]); } puts("");
}
...
int _set_validation(char* v[], int f, int t) {
   	...
   	if (f > t) {
        	for (int j = f - 1; j > t; --j) {
            		for (k = 0; k < 64; ++k) if (!strcmp(AN[k], v[j])) 		break;
            		if (player) {
                		if      (board[k] > BLACK_PIECES_VALUE_MAX)		return 1;
                		else if (board[k] && board[k] < WHITE_PIECES_VALUE_MIN) return k;
            		}
            		if (!player) {
               			if      (board[k] && board[k] < WHITE_PIECES_VALUE_MIN)	return 1;
                		else if (board[k] > BLACK_PIECES_VALUE_MAX) 		return k;
           		}
        	}
    	}
    	else {
                		...
                		for (int j = 0; j < 128; ++j) {
                    			if (!strcmp(AN_DIAGONAL[j], t)) {
                        			if   ((j - i) < -8 || (j - i) > 8) continue;
                       				else return _set_validation(AN_DIAGONAL, i, j);
                    			}
               			}
            		}
        	}
